...
class GmailExtractor:
    ...
    def get_message_list(self, query: str = "", page_token: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Normalize error messages to align with tests and provide guidance
            base_msg = "Failed to retrieve message list"
            if "Gmail API has not been used" in str(e) or "disabled" in str(e):
                raise ExtractionError(
                    f"{base_msg}: Gmail API not enabled. Please:\n"
            ...
            elif getattr(e, 'resp', None) is not None and getattr(e.resp, 'status', None) == 403:
                raise ExtractionError(
                    f"{base_msg}: access denied. Check:\n"
                ...
                raise ExtractionError(f"{base_msg}: {e}")
