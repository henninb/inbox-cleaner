...
class SpamFilterManager:
    ...
    @staticmethod
    def _criteria_key(filter_item: Dict[str, Any]) -> tuple:
        return tuple(sorted(filter_item.get('criteria', {}).items()))
    ...
    def filter_out_duplicates(self, new_filters: List[Dict[str, Any]], existing_filters: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        existing_keys = {self._criteria_key(f) for f in existing_filters}
        return [f for f in new_filters if self._criteria_key(f) not in existing_keys]
    ...
    def identify_duplicate_filters(self, filters: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for filter_item in filters:
            criteria_groups[self._criteria_key(filter_item)].append(filter_item)

        return [
            {'criteria': dict(key), 'filters': group, 'count': len(group)}
            for key, group in criteria_groups.items()
            if len(group) > 1
        ]
